...
summary: 'Demonstrated with Postman'
...
<Image
    alt={`JSON-RPC request example`}
    src={`/blog-images/json-rpc-requests-on-postman-with-jackson-and-spring/request-example.png`}
    width={1600}
    height={840}
/>
...
Positional means that you do not have to specify any keys but the parameters in your request must be in the same order as the arguments are specified in.
...
By default, we can use the `@c` annotation which is where Jackson uses the minimal path of the class name as the type identifier (i.e. the setting `@JsonTypeInfo` has defaulted to `JsonTypeInfo.Id.MINIMAL_CLASS`).
